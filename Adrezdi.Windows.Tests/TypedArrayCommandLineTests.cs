...
namespace Adrezdi.Windows.Tests
{
	public abstract class TypedArrayCommandLineTests<T> : TypedCommandLineTests<T[]>
	{
		...
		protected new void FlagValuePropertyHasLongNameArgumentWithValue(T[] value)
		{
			// Arrange
			var args = new string[] { "--flag:" + string.Join(",", value) };
			var x = new CommandLine();
			...
			// Assert
			Assert.IsTrue(Enumerable.SequenceEqual(value, result.FlagValue));
		}

		protected new void FlagValuePropertyHasShortNameArgumentWithValue(T[] value)
		{
			// Arrange
			var args = new string[] { "-f:" + string.Join(",", value) };
			var x = new CommandLine();
		...
		protected new void FlagValuePropertyHasLongNameArgumentWithEmptyValue()
		{
			// Arrange
			...
			if(typeof(T) == typeof(string))
			{
				Assert.AreEqual(1, result.FlagValue.Length);
				Assert.AreEqual("", result.FlagValue[0].ToString());
			}
			else
				Assert.Fail();
		}

		protected new void FlagValuePropertyHasShortNameArgumentWithEmptyValue()
		{
			// Arrange
